<script lang="ts">
	...
	import { localGames, type LocalSession } from '$lib/stores/localGames';

	let { data } = $props();

	const supabase = createSupabaseClient();

	// For local games, load and create session from localStorage
	const localGame = $derived(data.isLocalGame ? localGames.getGame(data.gameId) : null);

	// Initialize local session for local games
	let localSession = $state<LocalSession | undefined>(undefined);
	$effect(() => {
		if (data.isLocalGame && localGame && !localSession) {
			localSession = localGames.createSession(data.gameId);
		}
	});
	...
	const session = $derived(data.isLocalGame ? localSession : data.session);
	const initialScores = $derived(data.isLocalGame ? (localSession?.scores || []) : data.scores);
	...
	// Initialize scores from data
	$effect(() => {
		if (initialScores.length > 0 && scores.length === 0) {
	...
	async function saveScore(playerId: string, tricksWon: number) {
		...
		if (data.isLocalGame) {
			...
			localGames.updateScore(data.gameId, session.id, playerId, currentRound, tricksWon);
			isSaving = false;
		} else {
	...
	async function completeSession() {
		...
		if (data.isLocalGame) {
			localGames.completeSession(data.gameId, session.id);
			goto(`/games/${game.id}`);
		} else {
...
<svelte:head>
	<title>Round {currentRound} - {game?.name || 'Game'}</title>
</svelte:head>
	...
	{#if showSummary}
		...
		<div class="p-4 max-w-lg mx-auto">
			<header class="mb-6">
				...
				<p class="text-center text-gray-500 mt-2">{formatDate(session.played_at)}</p>
			</header>
	...
	{:else}
		...
		<div class="p-4 max-w-lg mx-auto pb-32">
			<header class="flex items-center justify-between mb-4">
				<button onclick={() => goto(`/games/${game.id}`)} class="p-2 -ml-2 text-gray-500 hover:text-gray-700">
					<svg
...
{:else}
	<div class="p-4 max-w-lg mx-auto text-center py-16">
		<h1 class="text-xl font-bold text-gray-900 mb-4">Loading...</h1>
		<p class="text-gray-500">Please wait while we load your session.</p>
	</div>
{/if}
